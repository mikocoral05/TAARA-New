...
export default {
  setup() {
    ...
    let getDays = () => {
      ...
      numberOfDays.value = new Date(
        filteFundsExpenses.value.year,
        month,
        0
      ).getDate();
      let formatted_month = ("0" + filteFundsExpenses.value.month.value).slice(
        -2
      );
      if (
        filteFundsExpenses.value.month.value ==
        monthTodayBasedScroll.value.value
      ) {
    ...
    let submitExpense = (addInfo) => {
      ...
      let fullDate =
        filteFundsExpenses.value.year + "-" + formatted_month + "-" + day.value;
    ...
    let allDonation = (result, monthlyTotalExpense) => {
      if (result[filteFundsExpenses.value.month.value - 1] !== undefined) {
        ...
        let filterOutput =
          monthDonation[0].net_donation +
          Number(monthDonation[0].total_expense);
