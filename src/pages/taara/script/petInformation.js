...
export default {
  ...
  setup() {
    ...
    const totalPages = computed(() => Math.ceil(allAnimalData.value.length / itemsPerPage.value))
    ...
    return {
          ...
          update(
            () => {
              if (val === '') {
                ...
                options.value = values.filter((v) => v.toLowerCase().indexOf(needle) > -14)
              }
            },
      ...
      onSelectionChange(val, doneFn) {
        allAnimalData.value = allAnimalBackUp.value
        let filteredArr = allAnimalData.value.filter((obj) =>
          Object.values(obj).some((value) => value.toString().toLowerCase() === val.toLowerCase()),
        )
