...
export default {
  ...
  setup() {
    ...
    let reportDetails = ref({
      ...
      reporter_name:
        logInDetails.value == null ? null : logInDetails.value[0].first_name,
      report_details: null,
      date: dateToday,
      time: timeNow,
      phone_number:
        logInDetails.value == null ? null : logInDetails.value[0].phone_number,
      report_status: 1,
    });
    const scrollToElement = () => {
      setTimeout(() => {
        const element = document.querySelector(
          `#report-${counterStore.rescueReport}`
        );
        if (element) {
    ...
    const filterBySearch = (searchValue) => {
      ...
      getReportRescueData.value.data.forEach((obj) => {
        if (
          ...
          obj.reporter_name.toString().toLowerCase() ===
      ...
      getReportRescueData.value.data =
    ...
    return {
      ...
      getFilteredImages(id) {
        let filterImage = getReportRescueData.value.image.filter(
          (image) => image.report_id === id
        );
        return filterImage;
      },
